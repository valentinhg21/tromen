...
function price_fee_html($product) {
    ...
    // Obtener el precio base (precio de oferta si existe, o precio regular si no hay oferta)
    $precio_base = $product->get_regular_price(); // Precio actual (considera precio de oferta si existe)
    $precio_regular = $product->get_regular_price(); // Precio regular

    // Determinar si el producto está en rebaja
    $is_on_sale = $product->is_on_sale();

    // Calcular los precios
    $precio_tres_cuotas = $precio_base / 3;
    $precio_tres_format = wc_price($precio_tres_cuotas);

    $precio_cuota = $precio_base * 1.13 / 6;
    $precio_cuota_format = wc_price($precio_cuota);

    $precio_doce_cuotas = $precio_base * 1.30 / 12;
    $precio_doce_format = wc_price($precio_doce_cuotas);

    // Solo mostrar el precio en un pago si el producto está en oferta
    $precio_efectivo_html = '';
    if ($is_on_sale) {
        $precio_efectivo_format = wc_price($precio_base); // Precio de oferta como precio en un pago
        $precio_efectivo_html = '<p class="efectivo">En 1 pago de ' . $precio_efectivo_format . '</p>';
    }

    // Obtener valores de ACF
    $tres_cuotas = get_field('3_cuotas', $product->get_id());
    $seis_cuotas = get_field('6_cuotas', $product->get_id());
    $doce_cuotas = get_field('12_cuotas', $product->get_id());

    // Generar HTML para cuotas
    $tres_html = '';
    if ($tres_cuotas) {
        $tres_html = '<p class="cuotas"><span>3</span> cuotas fijas de ' . $precio_tres_format . '</p>';
    }

    $seis_html = '';
    if ($seis_cuotas) {
        $seis_html = '
        <p class="cuotas"><span>6</span> cuotas fijas de ' . $precio_cuota_format . '</p>
        ';
    }
    ...
    if ($doce_cuotas) {
        $doce_html = '
        <p class="cuotas"><span>12</span> cuotas fijas de ' . $precio_doce_format . '</p>
        ';
    }
    ...
    return '
    <div class="price-fee' . $extra_class . '">
        ' . $tres_html . '
        ' . $seis_html . '
        ' . $doce_html . '
    </div>';
